...
<br><h1 class="text-center text-primary">JAVA Quiz</h1>
...
<br>
<form action="mockjavacheck.php" method="post">
    <?php
 $con = mysqli_connect('localhost','root','','quiz') or die('Unable To connect');
    for($i=1; $i<21; $i++){
        $sql="SELECT * FROM mjavaquestion where qid=$i";
        ...
        ?>
            ...
            <?php 
            $sql="SELECT * FROM mjavaanswer where ans_id=$i";
            $result = mysqli_query($con,$sql);
            $row='';
            while($row = mysqli_fetch_array($result)){
            ?>
