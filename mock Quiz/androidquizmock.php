...
<br><h1 class="text-center text-primary">Android Quiz</h1>
...
<br>
<form action="mockandroidcheck.php" method="post">
    <?php
 $con = mysqli_connect('localhost','root','','quiz') or die('Unable To connect');
    for($i=1; $i<21; $i++){
        $sql="SELECT * FROM mandroidquestion where qid=$i";
        ...
        ?>
            ...
            <?php 
            $sql="SELECT * FROM mandroidanswer where ans_id=$i";
            $result = mysqli_query($con,$sql);
            $row='';
            while($row = mysqli_fetch_array($result)){
            ?>
